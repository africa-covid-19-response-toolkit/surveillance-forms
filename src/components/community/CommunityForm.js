...
const CommunityForm = ({ onSubmit, lang }) => {
  ...
  const fields = [
    ...
    },
    {
      ...
      property: 'region',
      choices: REGION_KEYS.map(r => ({ label: lang.t(`region.${r}`), value: r })),
    ...
    return (
      <form autoComplete="off">
        ...
        <Grid container spacing={4}>
          <Grid item xs={12} md={3} >{renderFormField('firstName')}</Grid>
          <Grid item xs={12} md={3} >{renderFormField('lastName')}</Grid>
          <Grid item xs={12} md={3} >{renderFormField('age')}</Grid>
          <Grid item  xs={12} md={3} >{renderFormField('sex')}</Grid>
        ...
        <Grid container spacing={4}>
          <Grid item  xs={12} md={3} >{renderFormField('region')}</Grid>
          <Grid item  xs={12} md={3} >{renderFormField('subcityOrZone')}</Grid>
          <Grid item  xs={12} md={3} >{renderFormField('sefer')}</Grid>
          <Grid item  xs={12} md={3} >{renderFormField('woreda')}</Grid>
          <Grid item  xs={12} md={3} >{renderFormField('kebele')}</Grid>
          <Grid item  xs={12} md={3} >{renderFormField('houseNo')}</Grid>
        </Grid>
        ...
        <Grid container spacing={4}>
          <Grid item  xs={12} md={6} >{renderFormField('travelHx')}</Grid>
          <Grid item  xs={12} md={6} >{renderFormField('animalMarket')}</Grid>
          <Grid item  xs={12} md={6} >{renderFormField('haveSex')}</Grid>
          <Grid item  xs={12} md={6} >{renderFormField('healthFacility')}</Grid>
        </Grid>

        <Box mt={4} textAlign="right">
          <Button onClick={hadleSubmit} variant="contained" size="large" disabled={!isFormValid()}>{lang.t('submit')}</Button>
        </Box>
      </form>
    )
  };
